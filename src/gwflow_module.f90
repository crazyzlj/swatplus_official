      ...
      type groundwater_state
        ...
        integer :: hole = 0         !             |sinkhole flag (0=no sinkhole; 1=sinkhole is present)
        integer :: cdut = 0         !             |conduit flag (0=no conduit; 1=conduit is present)
      ...
      type groundwater_ss
        ...
        real :: hole = 0.           !              |sinkhole inflow
        real :: cdut = 0.           !              |conduit outflow
      ...
      !sinkhole: variables for sinkhole inflow
      integer :: gw_sinkhole_flag = 0                              !     |
      real :: gw_hole_bypass = 0.                                  !none |fraction of effective precipitation flow in to the sinkhole
      !hru-sinkhole connection
      integer, dimension(:), allocatable :: gw_sinkhole_hruflag    !none |indicate hru's effective precipitation flows into a sinkhole
      ...
      !sinkhole cell-conduit cells connection
      integer :: gw_sinkhole_count = 0                             !none |sinkhole number
      integer, dimension(:), allocatable :: gw_sinkhole_list       !none |sinkhole index -> sinkhole's valid cell id
      real, dimension (:), allocatable :: gw_sinkhole_vol          !m3   |temporary variable to summarize volume from HRUs
      type sinkhole_conduit_conn_info
        integer :: ncon = 0                                        !     |number of conduit cells connected to the sinkhole
        integer, allocatable :: cells(:)                           !     |conduit cells connected to the sinkhole
        real, allocatable :: fract(:)                              !none |fraction
      endtype sinkhole_conduit_conn_info
      type (sinkhole_conduit_conn_info), dimension(:), allocatable :: gw_hole_cdut_info
      ...
      real, dimension (:), allocatable :: gw_cdut_conddepth        !mm/day/m |equivalent conduit drainage depth per day generated by a 1 m groundwater head difference
      real, dimension (:), allocatable :: gw_cdut_exp              !none     |dimensionless exponent controlling the nonlinearity of conduit discharge response, ranges from 1.0 to 2.0
      real, dimension (:), allocatable :: gw_cdut_qmax             !mm/day   |maximum allowable conduit discharge from a grid cell to the connected channel within one day
      real, dimension (:), allocatable :: gw_cdut_stor             !m3       |"reservoir" of each conduit cell
      integer :: gw_conduit_count = 0                              !none     |conduit number
      integer, dimension(:), allocatable :: gw_conduit_list        !none     |conduit index -> conduit's valid cell id
      !channel-conduit connection
      ...
      type solute_ss
        ...
        real :: hole = 0.           !g            |solute mass entering cell via sinkhole inflow
        real :: cdut = 0.           !g            |solute mass leaving cell via conduit outflow
      ...
      real, dimension (:), allocatable :: sol_grid_chng_mo,sol_grid_rech_mo,sol_grid_gwsw_mo,sol_grid_swgw_mo, &
                                          ...
                                          sol_grid_hole_mo,sol_grid_cdut_mo, &
      ...
      real, dimension (:), allocatable :: sol_grid_hole_yr
      real, dimension (:), allocatable :: sol_grid_cdut_yr
      ...
      real, dimension (:), allocatable :: sol_grid_hole_tt
      real, dimension (:), allocatable :: sol_grid_cdut_tt
